...
// The paid requests themselves, shared with `demo.ts` so that "how a slot is
// bought" has one implementation rather than two that drift apart silently.
// Every value they need is passed in from the literals declared above.
import {
  attemptBuy,
  pullQuote,
  pullThroughTheHub,
...
describe('the devnet', () => {
  ...
  beforeAll(async () => {
    ...
    quote = await pullQuote(broadcaster, HUB_ADDRESS);
    ...
    refusedAtWrongPrefix = await attemptBuy(
      broadcaster,
      HUB_ADDRESS,
